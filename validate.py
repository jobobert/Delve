...
def validate_npcs(npcs, items, styles):
    ...
    for nid, npc in npcs.items():
        ...
        # Dialogue coverage — non-hostile NPCs that players can talk to should have a tree
        is_hostile  = npc.get("hostile", False)
        has_shop    = bool(npc.get("shop"))
        has_dlg_str = bool(npc.get("dialogue"))
        has_dlg_file = nid in dialogue_ids
        if not is_hostile and not has_dlg_file:
            if has_shop:
                warn(f"NPC '{nid}' has a shop but no dialogue tree — "
                     f"players can't access it via 'talk'")
            elif not has_dlg_str:
                warn(f"NPC '{nid}' is not hostile and has no dialogue tree or fallback string")
